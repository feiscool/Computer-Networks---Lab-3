...
int main(int argc, char *argv[]) {
    ...
    uint8_t RID_src; //this used to be RID_src
    ...
    while (1) { // repeatedly prompt the user for a ring ID RID and a message m.

      printf("\nEnter a RID to send your message to: \n");
      scanf("%d", &RID_dest);

      printf("\nEnter your message: \n");
      scanf("%s", message);

      while (sizeof(message) < 65) {
      ...
      struct packed_message packet = {GID, magicNumber_BigE, TTL, RID_dest, RID_src,
                                      &message, checksum};
